...
if ($method === 'POST') {
    ...
    if (empty($invoice_number) || empty($seal_number) || empty($declared_value) || empty($client_id) || empty($route_id) || empty($client_site_id) || empty($fund_id)) {
        http_response_code(400); // Bad Request
        echo json_encode(['success' => false, 'error' => 'Todos los campos (Planilla, Sello, Valor, Cliente, Ruta, Sede, Fondo) son requeridos.']);
        $conn->close();
        exit;
    }
    if ($declared_value <= 0) {
    ...
    $stmt = $conn->prepare("INSERT INTO check_ins (invoice_number, seal_number, declared_value, fund_id, client_id, route_id, client_site_id, checkinero_id, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'Pendiente')");
    ...
    $stmt->bind_param("ssdiissi", $invoice_number, $seal_number, $declared_value, $fund_id, $client_id, $route_id, $client_site_id, $checkinero_id);
